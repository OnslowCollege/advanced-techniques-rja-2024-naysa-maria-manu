...
def play_card(card_key):
    """Handle playing a card and update the game state."""
    ...
    if card_key in player_cards:
        try:
            ...
            if top_card:
                ...
                # Check for special case with +4 card
                if top_value == "+4":
                    # Computer already drew 4 cards, so allow any card
                    pass
                elif (
            ...
            # If player played +4, make the computer draw 4 cards
            if card_value == "+4":
                for _ in range(4):
